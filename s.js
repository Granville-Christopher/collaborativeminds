...
`;
  ...
  useEffect(() => {
    const checkStatus = async () => {
      const savedUserId = await AsyncStorage.getItem("myUserId");
      if (savedUserId) {
  ...
  const onWebViewMessage = async (event) => {
    try {
      ...
      // Android raw string fallback
      if (event.nativeEvent.data.length > 30) {
        await sendTokenToBackend(event.nativeEvent.data);
      }
    }
  };
  ...
  const sendTokenToBackend = async (token) => {
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        // PERSIST THE DATA FOR ANDROID
        await AsyncStorage.setItem("myUserId", result.user_id);
        setMyUserId(result.user_id);
        setIsLoggedIn(true);
        setShowWebView(false); // Close WebView manually
        Alert.alert("✅ Connected", "Join tracking is now active!");
      }
    } catch (error) {
